...
class Voxelization(nn.Module):
    ...
    def __init__(self,
        ...
        # expand axis
        self.lb_DHW = rearrange(torch.tensor([ z_min,y_min,x_min]),
                                "d->1 d")
        self.vox_sz_DHW = rearrange(torch.tensor(self.voxel_size_DHW),
                                    "d->1 d")
    ...
    def process_single_batch_pc(self, pc:torch.Tensor, ibatch):
        ...
        for i_vox in range(unique_coord.shape[0]):
            ind = torch.argwhere(inverse_ind==i_vox).to("cpu").numpy().flatten()
